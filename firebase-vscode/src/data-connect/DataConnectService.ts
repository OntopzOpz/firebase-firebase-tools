...
import { parseVariableString } from "./parseVariableString";
...
export class DataConnectService {
    ...
    async servicePath(
        ...
        const serviceId = dataConnectConfigsValue?.tryReadValue?.findEnclosingServiceForPath(path)?.value.serviceId;
    ...
    private async handleProdResponse(
        ...
        if (!(response.status >= 200 && response.status < 300)) {
            const errorResponse = response as ClientResponse<ExecuteGraphqlResponseError>;
